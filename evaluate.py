...
def roc_auc(scores: np.ndarray, labels: np.ndarray) -> float:
    """Exact ROC AUC via the Mann-Whitney U identity.

    AUC = U / (N_pos * N_neg), where U is computed from average ranks so
    tied scores between positives and negatives contribute 0.5 each
    (the academic definition, e.g. Hanley & McNeil 1982). This is
    equivalent to sklearn.metrics.roc_auc_score and is exact, not
    trapezoidal.
    """
    ...
    if n_pos == 0 or n_neg == 0:
        return float("nan")
    # Average ranks: ties get the mean of the consecutive ranks they occupy.
    # Implemented without scipy so this stays a leaf module.
    ...
    while i < len(sorted_scores):
        ...
        while j + 1 < len(sorted_scores) and sorted_scores[j + 1] == sorted_scores[i]:
            j += 1
        if j > i:
            avg = (i + j + 2) / 2.0  # mean of 1-indexed ranks (i+1)..(j+1)
